...
def do_deploy(archive_path):
    ...
    try:
        ...
        run(f"tar -xzf /tmp/{archive_file} -C /"
            f"data/web_static/releases/{archive_folder}/")
        run(f"rm /tmp/{archive_file}")
        run(f"mv /data/web_static/releases/{archive_folder}/"
            f"web_static/* /data/web_static/releases/{archive_folder}/")
        run(f"rm -rf /data/web_static/releases/{archive_folder}/web_static")
        run(f"rm -rf /data/web_static/current")
        run(f"ln -s /data/web_static/releases/"
            f"{archive_folder}/ /data/web_static/current")
        return True
    except Exception:
        return False
